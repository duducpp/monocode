...
describe("OMP command lifecycle over the real RPC multiplexer", () => {
  ...
  it.each([["pi", sendPiTurn], ["omp", sendOmpTurn]] as const)(
    ...
    async (flavor, send) => {
      ...
      const turn = send({ ...input(sessionId, "Run it"), model: `${flavor}:default` });
      await vi.waitFor(() => expect(transport.requests.some((r) => r.sessionId === sessionId && r.command.type === "prompt")).toBe(true));
      const text = (value: string) => ({ content: [{ type: "text", text: value }] });
      try {
        frame(sessionId, { type: "tool_execution_start", toolCallId: "sh", toolName: "bash", args: { command: "make" } });
        frame(sessionId, { type: "tool_execution_update", toolCallId: "sh", partialResult: text("building") });
        frame(sessionId, { type: "tool_execution_end", toolCallId: "sh", result: text("built"), isError: false });
        ...
        expect(row?.tool).toMatchObject({ status: "completed", detail: "built" });
        expect(events).toContainEqual(
          expect.objectContaining({ type: "tool.updated", status: "running", detail: "building" }),
        );
      } finally {
        frame(sessionId, { type: "agent_end" });
        await turn;
